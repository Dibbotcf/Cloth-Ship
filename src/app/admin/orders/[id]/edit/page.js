...
export default function EditOrderPage({ params }) {
    ...
    fetch(`/api/admin/orders/${resolvedParams.id}`)
      ...
      .then(data => {
        ...
        if (data.items && data.items.length > 0) {
          ...
          setItems([{ product_name: '', size: 'Free Size', quantity: 1, price: 0 }]);
        }
        setInitialLoading(false);
      })
      .catch(e => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      shipping_amount: parseFloat(formData.shipping_amount),
  ...
  if (initialLoading) {
    return <div style={{ padding: '24px', textAlign: 'center', color: '#666' }}>Loading order details...</div>;
  }
    ...
    <div>
      ...
      <div className={styles.pageHeader}>
        <div className={styles.pageTitleGroup}>
          ...
          <h1 className={styles.pageTitle}>Edit Order #{resolvedParams.id}</h1>
        </div>
      ...
      <form onSubmit={handleSubmit} className={styles.formCard}>
        ...
        <div className={styles.formActions}>
          <button type="submit" disabled={loading} className={styles.primaryBtn}>
            {loading ? '⏳ Saving…' : <><Check size={18} style={{ display: 'inline', verticalAlign: 'text-bottom', marginRight: '6px' }} /> Save Changes</>}
          </button>
